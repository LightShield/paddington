...
def find_aggregate_initializations(translation_unit: clang.TranslationUnit, struct_name: str) -> List[UsageSite]:
    ...
    def visit(cursor):
        ...
        if cursor.kind == clang.CursorKind.INIT_LIST_EXPR:
            ...
            if cursor.type.spelling == struct_name or cursor.type.spelling == f'struct {struct_name}':
...
def find_constructor_calls(translation_unit: clang.TranslationUnit, struct_name: str) -> List[UsageSite]:
